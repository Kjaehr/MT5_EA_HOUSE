...
#property description "GOLD Liquidity Sweep Strategy EA"

#include <Trade/Trade.mqh>

//--- Input parameters
input group "=== STRATEGY SETTINGS ==="
input int Inp_Magic = 55221005;                    // Magic Number (Sweep)
input string Inp_EA_Tag = "XAU_Sweep";            // EA Tag
...
input int Inp_Sweep_Lookback = 80;                // Lookback bars for swing points
input int Inp_Sweep_MinWickPts = 20;              // Minimum wick size in points
input int Inp_Sweep_SL_Buffer = 15;               // Stop Loss buffer in points
input double Inp_Sweep_SL_ATR = 0.8;              // Stop Loss ATR multiplier
...
int OnInit() {
    ...
    Print("Liquidity Sweep EA initialized successfully. Magic: ", Inp_Magic);
...
void OnDeinit(const int reason) {
    Print("=== LIQUIDITY SWEEP EA DEINITIALIZATION SUMMARY ===");
...
void OnTradeTransaction(const MqlTradeTransaction& trans,
    ...
    if(entry_type == DEAL_ENTRY_OUT || entry_type == DEAL_ENTRY_INOUT) {
        ...
        if(Inp_LogVerbose) {
            Print("Liquidity Sweep Trade closed: Ticket=", ticket, " Profit=", DoubleToString(profit, 2),
...
void InitCSV() {
    ...
    if(g_csv_handle != INVALID_HANDLE) {
        FileWrite(g_csv_handle, "timestamp", "action", "ticket", "symbol", "type", "lots", "price",
                 "sl", "tp", "profit", "comment", "strategy", "session", "sweep_level", "atr");
...
void WriteTradeToCSV(ulong ticket, string action, double price, double lots, string comment) {
    ...
    TradeMeta meta;
    string strategy = "LIQUIDITY_SWEEP";
    string session = "UNKNOWN";
    double sl = 0, tp = 0, profit = 0, sweep_level = 0;

    if(GetTradeMeta(ticket, meta)) {
        session = EnumToString(meta.session);
        sl = meta.sl_price;
        tp = meta.tp_price;
        sweep_level = meta.sweep_level;
    }
    ...
    FileWrite(g_csv_handle,
        ...
        strategy,
        session,
        DoubleToString(sweep_level, _Digits),
...
//+------------------------------------------------------------------+
//| Liquidity Sweep Strategy Functions                              |
//+------------------------------------------------------------------+
bool IsEligible(SessionId session) {
...
bool Arm(SessionId session) {
    ...
    double current_close = iClose(_Symbol, PERIOD_M1, 0);
    double sl_pts = g_cache.ATR * Inp_Sweep_SL_ATR;

    // Calculate lot size (include buffer in SL calculation)
    double total_sl_pts = sl_pts + Inp_Sweep_SL_Buffer * _Point;
    double lots = CalculateLotSize(Inp_RiskPct, Pts(total_sl_pts));
    if(lots <= 0) return false;

    bool success = false;
    ulong ticket = 0;

    if(current_close < g_sweep_level) {
        // Sweep above - sell the reversion
        double sl_price = g_sweep_level + total_sl_pts;
        double tp_price = g_sweep_level - g_cache.ATR; // Target one ATR below sweep level

        if(g_trade.Sell(lots, _Symbol, 0, sl_price, tp_price, Inp_EA_Tag + "_SWEEP_SELL")) {
            ticket = g_trade.ResultOrder();
            success = true;

            // Log to CSV
            if(Inp_WriteCSV) {
                WriteTradeToCSV(ticket, "SELL", current_price, lots, "SWEEP_ABOVE_REVERSION");
            }
        }
    }
    else if(current_close > g_sweep_level) {
        // Sweep below - buy the reversion
        double sl_price = g_sweep_level - total_sl_pts;
        double tp_price = g_sweep_level + g_cache.ATR; // Target one ATR above sweep level

        if(g_trade.Buy(lots, _Symbol, 0, sl_price, tp_price, Inp_EA_Tag + "_SWEEP_BUY")) {
            ticket = g_trade.ResultOrder();
            success = true;

            // Log to CSV
            if(Inp_WriteCSV) {
                WriteTradeToCSV(ticket, "BUY", current_price, lots, "SWEEP_BELOW_REVERSION");
            }
        }
    }
    ...
    if(success) {
        // Store trade meta
        TradeMeta meta;
        meta.strategy_short = "LIQUIDITY_SWEEP";
        meta.session = session;
        meta.lots = lots;
        meta.open_price = current_price;
        meta.sl_price = current_close < g_sweep_level ? g_sweep_level + total_sl_pts : g_sweep_level - total_sl_pts;
        meta.tp_price = current_close < g_sweep_level ? g_sweep_level - g_cache.ATR : g_sweep_level + g_cache.ATR;
        meta.remaining_lots = lots;
        meta.open_time = TimeCurrent();
        meta.sweep_level = g_sweep_level;
        StoreTradeMeta(ticket, meta);

        g_active_ticket = ticket;

        if(Inp_LogVerbose) {
            Print("Liquidity Sweep Armed: Level=", DoubleToString(g_sweep_level, _Digits),
                  " Current=", DoubleToString(current_price, _Digits),
                  " Direction=", current_close < g_sweep_level ? "SELL" : "BUY",
                  " ATR=", DoubleToString(g_cache.ATR, _Digits),
                  " Ticket=", ticket);
        }
        return true;
    }
...
void OnTickActive() {
    ...
    // For liquidity sweep, we typically just wait for TP/SL
}
...
void OnExit(bool profit) {
    ...
    if(Inp_LogVerbose) {
        Print("Liquidity Sweep Exit: ", profit ? "Profit" : "Loss",
