...
input double   LotSize = 0.1;                    // Base position size
...
input double   MinVolumeMultiplier = 0.02;        // Minimum volume vs average (backtest adjusted)
...
input double   MinTickVelocity = 0.5;            // Minimum tick velocity threshold
input bool     UseZScoreAnalysis = false;        // Use Z-Score statistical arbitrage
input int      ZScorePeriod = 20;                // Period for Z-Score calculation
input double   ZScoreThreshold = 1.5;            // Z-Score threshold for mean reversion
...
void AnalyzeAndTrade()
{
    ...
    if(signal != 0)
    {
        Print("TRADE EXECUTION: Signal=", signal, " | Quality=", signal_quality, " | UsePullbackEntries=", UsePullbackEntries);

        // Check for pullback entry if enabled
        if(IsPullbackEntry(market_condition, signal))
        {
            Print("EXECUTING TRADE: Pullback entry confirmed");
        ...
        else
        {
            ...
            Print("EXECUTING TRADE: Pullback entries disabled, executing immediately");
            ExecuteProfessionalTrade(signal, signal_quality);
        }
    }
}
...
double AnalyzeMomentumQuality()
{
    ...
    if(Bars(_Symbol, PERIOD_M1) < 10)
        return 0.5; // Return neutral quality if not enough data

    int copied = CopyClose(_Symbol, PERIOD_M1, 0, 10, close);
    if(copied < 10)
    {
        ResetLastError();
        return 0.5; // Return neutral quality if copy fails
    }

    // Validate close prices to avoid division by zero
    if(close[2] == 0.0 || close[5] == 0.0)
        return 0.5; // Return neutral quality if invalid prices
...
int GetProfessionalSignal(ENUM_MARKET_CONDITION market_condition)
{
    ...
    if(market_condition == MARKET_RANGING)
    {
        ...
        if(range_size > g_current_volatility * 1.5) // Reduced threshold for backtest data
        {
            ...
            // Debug range position
            if(TimeCurrent() - last_range_debug > 300)
            {
                Print("RANGE POSITION: ", range_position, " (0=support, 1=resistance)");
            }

            // Buy near support with additional confirmations
            if(range_position <= 0.25) // Tightened from 0.15
            {
                bool oversold = UseZScoreAnalysis ? CheckZScoreCondition(1) : IsOversoldCondition();
                if(TimeCurrent() - last_range_debug > 300)
                {
                    Print("BUY CHECK: Range pos=", range_position, " | Oversold=", oversold, " | Volume confirmed=", volume_confirmed);
                    if(UseZScoreAnalysis)
                    {
                        double z_score = CalculateZScore();
                        Print("BUY Z-SCORE: ", z_score, " | Threshold: -", ZScoreThreshold);
                    }
                }

                // Additional confluence: check for oversold conditions (Z-Score or traditional)
                if(oversold && volume_confirmed)
                {
                    // Check momentum if enabled
                    if(UseMomentumCheck && !CheckMomentumAlignment(1))
                        return 0; // Momentum not aligned for BUY

                    // Check tick velocity if enabled
                    if(UseTickVelocity && !CheckTickVelocityAlignment(1))
                        return 0; // Tick velocity not sufficient for BUY
            ...
            {
                bool overbought = UseZScoreAnalysis ? CheckZScoreCondition(-1) : IsOverboughtCondition();
                if(TimeCurrent() - last_range_debug > 300)
                {
                    Print("SELL CHECK: Range pos=", range_position, " | Overbought=", overbought, " | Volume confirmed=", volume_confirmed);
                    if(UseZScoreAnalysis)
                    {
                        double z_score = CalculateZScore();
                        Print("SELL Z-SCORE: ", z_score, " | Threshold: ", ZScoreThreshold);
                    }
                }

                // Additional confluence: check for overbought conditions (Z-Score or traditional)
                if(overbought && volume_confirmed)
                {
                    // Check momentum if enabled
                    if(UseMomentumCheck && !CheckMomentumAlignment(-1))
                        return 0; // Momentum not aligned for SELL

                    // Check tick velocity if enabled
                    if(UseTickVelocity && !CheckTickVelocityAlignment(-1))
                        return 0; // Tick velocity not sufficient for SELL
        ...
        else
        {
            if(TimeCurrent() - last_range_debug > 300)
            {
                Print("RANGE TOO SMALL: ", range_size, " <= ", g_current_volatility * 1.5);
            }
        }
    }
        ...
        if(g_current_volume_ratio >= MinVolumeMultiplier * 1.2) // Reduced from 1.8
        {
            ...
            if(momentum_quality > 0.6)
            {
                   ...
                   current_price > g_resistance_level + (g_current_volatility * 0.5))
                {
                    // Check momentum if enabled
                    if(UseMomentumCheck && !CheckMomentumAlignment(1))
                        return 0; // Momentum not aligned for BUY breakout

                    // Check tick velocity if enabled
                    if(UseTickVelocity && !CheckTickVelocityAlignment(1))
                        return 0; // Tick velocity not sufficient for BUY breakout
                   ...
                   current_price < g_support_level - (g_current_volatility * 0.5))
                {
                    // Check momentum if enabled
                    if(UseMomentumCheck && !CheckMomentumAlignment(-1))
                        return 0; // Momentum not aligned for SELL breakdown

                    // Check tick velocity if enabled
                    if(UseTickVelocity && !CheckTickVelocityAlignment(-1))
                        return 0; // Tick velocity not sufficient for SELL breakdown
...
bool IsOversoldCondition()
{
    ...
    // Consider oversold if 3+ of last 5 bars were declining (less strict for backtest)
    return declining_bars >= 3;
}
...
bool IsOverboughtCondition()
{
    ...
    // Consider overbought if 3+ of last 5 bars were rising (less strict for backtest)
    return rising_bars >= 3;
}

//+------------------------------------------------------------------+
//| Check momentum alignment with signal direction                 |
//+------------------------------------------------------------------+
bool CheckMomentumAlignment(int signal_direction)
{
    if(!UseMomentumCheck)
    ...
    // Ensure we have enough bars for momentum check
    int bars_needed = MomentumBars + 1;
    if(Bars(_Symbol, PERIOD_M1) < bars_needed)
    {
        Print("WARNING: Not enough bars for momentum check. Need: ", bars_needed, " Available: ", Bars(_Symbol, PERIOD_M1));
        return true; // Allow trade if not enough data (fail-safe)
    }

    // Copy close prices with proper error handling
    int copied = CopyClose(_Symbol, PERIOD_M1, 0, bars_needed, close);
    if(copied < bars_needed)
    {
        Print("ERROR: Failed to copy close prices for momentum check. Copied: ", copied, " Needed: ", bars_needed, " Error: ", GetLastError());
        ResetLastError();
        return true; // Allow trade if data copy fails (fail-safe)
    }

    // Check if momentum aligns with signal direction
    if(signal_direction > 0) // BUY signal
    {
        // Require upward momentum: current price > price X bars ago
        bool momentum_up = close[0] > close[MomentumBars];
        if(!momentum_up)
        {
            Print("DEBUG: BUY momentum check failed. Current: ", close[0], " vs ", MomentumBars, " bars ago: ", close[MomentumBars]);
        }
        return momentum_up;
    }
    else if(signal_direction < 0) // SELL signal
    {
        // Require downward momentum: current price < price X bars ago
        bool momentum_down = close[0] < close[MomentumBars];
        if(!momentum_down)
        {
            Print("DEBUG: SELL momentum check failed. Current: ", close[0], " vs ", MomentumBars, " bars ago: ", close[MomentumBars]);
        }
...
{
    if(!UseZScoreAnalysis)
        return 0.0; // Return neutral if disabled

    double close[];
    ArraySetAsSeries(close, true);

    // Ensure we have enough bars for Z-Score calculation
    if(Bars(_Symbol, PERIOD_M1) < ZScorePeriod + 1)
    {
        Print("WARNING: Not enough bars for Z-Score calculation. Need: ", ZScorePeriod + 1, " Available: ", Bars(_Symbol, PERIOD_M1));
        return 0.0; // Return neutral if not enough data
    }

    // Copy close prices with proper error handling
    int copied = CopyClose(_Symbol, PERIOD_M1, 0, ZScorePeriod + 1, close);
    if(copied < ZScorePeriod + 1)
    {
        Print("ERROR: Failed to copy close prices for Z-Score. Copied: ", copied, " Needed: ", ZScorePeriod + 1, " Error: ", GetLastError());
        ResetLastError();
        return 0.0; // Return neutral if data copy fails
    }

    // Calculate rolling mean (excluding current bar)
    double sum = 0.0;
    ...
    double std_dev = MathSqrt(variance_sum / ZScorePeriod);

    // Avoid division by zero
    if(std_dev == 0.0)
    {
        Print("WARNING: Standard deviation is zero for Z-Score calculation");
        return 0.0;
    }
    ...
    static datetime last_zscore_debug = 0;
    if(TimeCurrent() - last_zscore_debug > 300) // Debug every 5 minutes
    {
        Print("Z-SCORE DEBUG: Current=", current_price, " | Mean=", mean, " | StdDev=", std_dev, " | Z-Score=", z_score);
        last_zscore_debug = TimeCurrent();
    }
...
//+------------------------------------------------------------------+
//| Check Z-Score conditions for mean reversion signals           |
//+------------------------------------------------------------------+
bool CheckZScoreCondition(int signal_direction)
{
    if(!UseZScoreAnalysis)
        return true; // Skip check if disabled
    ...
    if(signal_direction > 0) // BUY signal
    {
        ...
        bool oversold_zscore = z_score <= -ZScoreThreshold;

        if(!oversold_zscore)
        {
            Print("DEBUG: Z-Score not oversold for BUY. Z-Score: ", z_score, " > -", ZScoreThreshold);
        }

        return oversold_zscore;
    }
    else if(signal_direction < 0) // SELL signal
    {
        // SELL when price is significantly above mean (overbought)
        bool overbought_zscore = z_score >= ZScoreThreshold;

        if(!overbought_zscore)
        {
            Print("DEBUG: Z-Score not overbought for SELL. Z-Score: ", z_score, " < ", ZScoreThreshold);
        }

        return overbought_zscore;
    }

    return true; // No signal direction specified
}
...
{
    Print("=== EXECUTING PROFESSIONAL TRADE ===");
    ...
    if(lot_size <= 0)
    {
        Print("ERROR: Invalid lot size, trade cancelled");
        return;
    }
...
void DebugSignalQuality()
{
    ...
    // Additional momentum debug info
    Print("Momentum Check Enabled: ", UseMomentumCheck);
    if(UseMomentumCheck)
    {
        Print("Momentum Bars: ", MomentumBars);
        double momentum_quality = AnalyzeMomentumQuality();
    ...
    // Additional Z-Score debug info
    Print("Z-Score Analysis Enabled: ", UseZScoreAnalysis);
    if(UseZScoreAnalysis)
    {
        Print("Z-Score Period: ", ZScorePeriod);
        Print("Z-Score Threshold: ", ZScoreThreshold);
        double current_zscore = CalculateZScore();
        Print("Current Z-Score: ", current_zscore);

        // Interpret Z-Score
        string zscore_interpretation = "";
        if(current_zscore >= ZScoreThreshold)
            zscore_interpretation = "OVERBOUGHT (SELL signal)";
        else if(current_zscore <= -ZScoreThreshold)
            zscore_interpretation = "OVERSOLD (BUY signal)";
        else
            zscore_interpretation = "NEUTRAL";
        Print("Z-Score Interpretation: ", zscore_interpretation);
    }

    // Volume confirmation check
    bool volume_confirmed = g_current_volume_ratio >= MinVolumeMultiplier;
    Print("Volume Confirmed: ", volume_confirmed, " (", g_current_volume_ratio, " >= ", MinVolumeMultiplier, ")");
...
double CalculatePositionSize(ENUM_SIGNAL_QUALITY quality)
{
    ...
    Print("POSITION SIZE DEBUG:");
    Print("Account Balance: ", account_balance);
    Print("Risk Percent: ", AccountRiskPercent, "%");
    ...
    if(g_consecutive_losses >= 1)
    {
        ...
        Print("Consecutive losses: ", g_consecutive_losses, " - Risk reduced by 50%");
    }
    ...
    Print("Point: ", point, " | Tick Value: ", tick_value);
    Print("SL Distance: ", sl_distance, " | SL Amount: ", sl_amount);

    double calculated_lots = (sl_amount > 0) ? risk_amount / sl_amount : LotSize;
    Print("Calculated lots (before limits): ", calculated_lots);
    ...
    Print("Symbol limits - Min: ", min_lot, " | Max: ", max_lot, " | Step: ", lot_step);
    Print("Base LotSize: ", LotSize, " | Max allowed: ", LotSize * 3.0);
    ...
    Print("Final calculated lots: ", calculated_lots);
